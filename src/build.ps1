...
$OutputPath = "$BuildFolder\output\UniversalDashboard.UDButtonParticle"
...
Copy-Item $BuildFolder\UniversalDashboard.UDButtonParticle.psm1 $OutputPath
Copy-Item $BuildFolder\Scripts $OutputPath\Scripts -Recurse -Force

$Version = "1.0.0"

$manifestParameters = @{
	Path = "$OutputPath\UniversalDashboard.UDButtonParticle.psd1"
	Author = "xxx"
	CompanyName = "Ironman Software, LLC"
	Copyright = "2019 Ironman Software, LLC"
	RootModule = "UniversalDashboard.UDButtonParticle.psm1"
	Description = ""
	ModuleVersion = $Version
	Tags = @("universaldashboard")
	ReleaseNotes = "xxx"
	FunctionsToExport = @(
		"New-UDButtonParticle"
	)
}
